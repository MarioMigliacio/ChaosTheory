...
void AssetManager::Init(const Settings& settingsRef) {
    ...
    if (!fallbackFont.loadFromFile(fallbackPath)) {
        CT_LOG_ERROR("Failed to load fallback font from '{}'. Text rendering may be broken!", fallbackPath);
    } else {
...
sf::Font& AssetManager::GetFont(const std::string& filename) {
    ...
    if (!font.loadFromFile(filename)) {
        CT_LOG_WARN("Failed to load font '{}'. Using fallback font instead.", filename);
        return fallbackFont;
    } else {
